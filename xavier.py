...
class Net(nn.Module):
    def __init__(self):
        ...
        self.layer1 = nn.Linear(100, 150)
        self.layer2 = nn.Linear(150, 200)
        self.layer3 = nn.Linear(200, 256)
...
def player1Action(board: list, remaining_ships : list):
    ...
    # ships_to_hits = {0: 2, 1: 3, 2:4, 3:4, 4:5}
    ...
    model.load_state_dict(torch.load(r"C:\\Users\\hsken\\OneDrive\\Documents\\C_projects\\Battleships\\bestoneyet.pth"))
    model.eval()

    print("Here")

    input_tensor = torch.from_numpy(board_input).unsqueeze(0).unsqueeze(0).float()  # shape: [1, 1, 10, 10]
    # input_tensor = torch.from_numpy(board_input).unsqueeze(0).view(1,100).float()
