...
def player1Action(board: list, remaining_ships: list, history: list):
    ...
    with open("players/model_harrish.pkl", "rb") as f:
        model = pickle.load(f)

    prediction = model.predict(input_board)
    output = torch.tensor(prediction.reshape(10, 10))

    tried = set((h[0], h[1]) for h in history)
    ...
    flat_idx = torch.argmax(output_masked).item()
    row = flat_idx // 10
    col = flat_idx % 10
